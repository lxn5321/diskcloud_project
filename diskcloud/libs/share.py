def generate_id(username, path, name, id_life):
    ...
    result_sel = select_execute("select expire_time,sid from storage where username = %s and path = %s and name = %s and trash_can = %s",(username, path, name, 0))
    if result_sel[0][0] != None and result_sel[0][1] != None:
        ...
        if result_life == 'permanent':
            ...
            result_update = update_execute('update storage set share_time = %s, expire_time = %s where username = %s and path = %s and name = %s and trash_can = %s',(now_time_str(), expire_time_str, username, path, name, 0))
            if result_update:
                db_commit()
                return generate_id_return(True,result_sel[0][1])
            db_rollback()
            ...
            result_update = update_execute('update storage set share_time = %s, expire_time = %s, sid = %s where username = %s and path = %s and name = %s and trash_can = %s',(now_time_str(), expire_time_str, sid, username, path , name, 0))
            if result_update:
                db_commit()
    ...
    sid = generate_random_str(8)
    result_update = update_execute('update storage set share = %s, share_time = %s, expire_time = %s, sid = %s where username = %s and path = %s and name = %s and trash_can = %s', (1 , now_time_str(), expire_time_str, sid, username, path , name, 0))
    if result_update:
        db_commit()
        return generate_id_return(True,sid)
    db_rollback()
...
def get_sid(username, path, name):
    ...
    result = select_execute('select sid from storage where username = %s and path = %s and name = %s and trash_can = %s', (username, path, name, 0));
    sid = result[0][0]
    if sid != None:
        return {'sid': sid}
    else:
...
def unshare(username, path, name):
    ...
    result = update_execute('update storage set share = %s, share_time = %s, expire_time = %s, sid = %s where username = %s and path = %s and name = %s and trash_can = %s', (0, None, None, None, username, path, name, 0))
    if result:
        db_commit()
        return ('', 200)
    db_rollback()
...
def valid_sid(sid):
    ...
    if re_match('[a-zA-Z0-9]{8}',sid):
        result = select_execute('select expire_time,username,path,name from storage where sid = %s and trash_can = %s',(sid, 0))
        if len(result) != 0:
